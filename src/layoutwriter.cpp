...
namespace {
	...
	string const rotation_to_axis_pyems_python(short const& r,
						const bool excited) {
		ostringstream axis_ss;
		switch (r) {
			case 0:
				axis_ss << "Axis(\"x\", direction="
					<< (excited ? 1 : -1) << ")";
				break;
			case 90:
				axis_ss << "Axis(\"y\", direction="
					<< (excited ? -1 : 1) << ")";
				break;
			case 180:
				axis_ss << "Axis(\"x\", direction="
			...
			case 270:
				...
				break;
			default:
				log_err << "WARNING : Invalid rotation angle.  Assumed r=0\n";
				axis_ss << "Axis(\"x\", direction="
					<< (excited ? 1 : -1) << ")";
...
int LayoutWriter::write(Block& block, long double const offset_x, long double const offset_y, string const& n_out, string const& name, vector<string>* out_names) const {
	...
	else if(data.out_format==".py") write_pyems_python(block, f_out, offset_x, offset_y, name);
...
//******************************************************************************
void LayoutWriter::write_pyems_python(Block& block, std::ofstream& f_out, long double const offset_x, long double const offset_y, std::string const& name) const {
	...
	sort(begin(ports), end(ports));
	// Does it make sense to continue if no ports defined?
	int port_default;
	ostringstream port_indicies_ss;
	if (ports.empty()) {
		port_default = 1;
	} else {
		port_default = ports[0].first;
		for (auto it = ports.begin(); it != ports.end()-1; ++it) {
			port_indicies_ss << it->first << ", ";
		}
		port_indicies_ss << ports.back().first;
	}

	f_out << "#!/usr/bin/env python3\n"
		"\n"
		"#### pyems script generated by Qucs-RFlayout from : " << name << ".sch\n"
		"\n"
		"from argparse import ArgumentParser\n"
		"\n"
		"from CSXCAD.CSTransform import CSTransform\n"
		"\n"
		"from pyems.const import C0, EPS0\n"
		"from pyems.coordinate import Axis, Box3, Coordinate2, Coordinate3\n"
		...
		"from pyems.structure import Microstrip, PCB\n"
		"from pyems.simulation import Simulation\n"
		"\n"
		"import numpy as np\n"
		"\n";

	f_out << "# offset_x = " << offset_x << " offset_y = " << offset_y << "\n\n";

	f_out << "####################################################### Pre-processing\n"
	         "\n";

	f_out << "parser = ArgumentParser(description='OpenEMS Octave script "
		"generated by Qucs-RFlayout from : " << name << "')\n";

	f_out << "parser.add_argument('--port', dest='ports', nargs='+', type=int,\n"
		"    action='append', default=[" << port_default << "],\n"
		"    help=\"Enable a port. \'N\' is a port number. By default the port "
		<< port_default << " is enabled.\")\n\n";

	f_out << "args = parser.parse_args()";
	  //	f_out << "parser.add_argument('--no-port', dest='noports', nargs='*', action='append',\n"
	  //		"    help='Disable a port. 'N' is a port number. By default all other ports are disabled."
	  //		"  Possible port numbers are : " << port_indicies_ss.str() << "\n\n"
	...
	f_out << "f0 = (fstop + fstart) / 2  # Center frequency\n"
		"fc = (fstop - fstart) / 2  # Cutoff frequency\n"
		"unit = 1e-3\n"
		"lambda0 = C0 / (f0 + fc) / unit\n"
		"high_div = " << data.oems_highres_div << "        # (currrently ignored for pyems) Depending on your simulation, you may want to tweak this value\n"
		"metal_div = " << data.oems_metalres_div << "        # Depending on your simulation, you may want to tweak this value\n"
		"substrate_div = "<< data.oems_substres_div << "    # Depending on your simulation, you may want to tweak this value\n"
		"time_res = " << data.oems_timeres << "     # Depending on your simulation, you may want to tweak this value\n"
		"high_res = lambda0 / high_div\n"
		"metal_res = lambda0 / metal_div\n"
		"substrate_res = lambda0 / substrate_div\n"
		"end_criteria = " << data.oems_end_criteria << "\n"
		"csx_file = '" << name << ".csx'\n"
		"\n";

	f_out << "# Simulation parameters \n"
	         "\n"
	...
	//vector<string> substrate_labels();
	for(shared_ptr<Element> it : block.elements) {
		type=it->getType();
		if(type=="SUBST") {
			f_out << "# " << it->getLabel() << " : " << type << "\n" <<
				it->getLabel() << "_ground_plane_t = 0.0175\n" <<
				it->getLabel() << "_metal_t = " << it->getT() << "\n" <<
				it->getLabel() << "_metal_rho = " << it->getRho() << "\n" <<
				it->getLabel() << "_metal_cond = 1 / " << it->getLabel() << "_metal_rho\n" <<
			        it->getLabel() << "_metal_center = Coordinate3(" << it->getX()+offset_x << ", " <<
				-(it->getY()+offset_y) << ", 0)\n" <<
			        it->getLabel() << "_dielectric_tand = " << it->getTand() << "\n" <<
				it->getLabel() << "_dielectric_er = " << it->getEr() << "\n" <<
				it->getLabel() << "_dielectric_K = " << it->getLabel() << "_dielectric_tand * 2 "
				"* np.pi * f0 * EPS0 * " << it->getLabel() << "_dielectric_er\n" <<
				it->getLabel() << "_dielectric_h = " << it->getH() << "\n" <<
				it->getLabel() << "_substrate_length = " << it->getL() << "\n" <<
				it->getLabel() << "_substrate_width = " << it->getW() << "\n" <<
				"\n"
				"pcb_length = " << it->getLabel() << "_substrate_length\n"
				"pcb_width = " << it->getLabel() << "_substrate_width\n"
				"# transformed position of PCB center\n"
			  //				"qucs_offset = Coordinate3(-(60.93 - 1.9)/2.0, -(-1.9 + -84.4001)/2.0, 0)\n"
				"\n" <<
				it->getLabel() << "_dielectric = Dielectric(epsr=[(1, " << it->getLabel() << "_dielectric_er)],\n"
				"    loss=[(1, " << it->getLabel() << "_dielectric_tand)])\n"
				"\n" <<
				it->getLabel() << "_pcb_prop = PCBProperties(\n"
				"    substrate=" << it->getLabel() << "_dielectric,\n"
				"    copper_thickness=[" << it->getLabel() << "_metal_t*unit, " <<
				it->getLabel() << "_ground_plane_t*unit],\n"
				"    substrate_thickness=[" << it->getLabel() << "_dielectric_h*unit],\n"
				"    metal_conductivity=" << it->getLabel() << "_metal_cond,\n"
				"    via_plating_thickness=0.0254e-3)\n"
				"    \n"
				"\n" <<
				it->getLabel() << "_pcb = PCB(\n"
				"    sim=sim,\n"
				"    pcb_prop=" << it->getLabel() << "_pcb_prop,\n"
				"    length=pcb_length,\n"
				"    width=pcb_width,\n"
				"    position=" << it->getLabel() << "_metal_center,\n"
				"    layers=range(3),\n"
				"    omit_copper=[0])\n"
				"\n"
				"\n";
			  //  				"translate = CSTransform()\n"
			  //				"translate.AddTransform(\"Translate\", qucs_offset.coordinate_list())\n";
	...
	for(shared_ptr<Element> it : block.elements) {
		if(!it->getActive())
			continue;
		type=it->getType();
		label_lower = it->getLabel();
		transform(label_lower.begin(), label_lower.end(), label_lower.begin(), ::tolower);
		if(type=="MGAP" || type=="MOPEN" || type=="MSTEP") {
			//nothing to do
		} else if(type=="MCORN"
			|| type=="MLIN") {
			...
			f_out << "# " << it->getLabel() << " : " << type << "\n"
			      << "center = Coordinate2(" << it->getX()+offset_x << ", " << -(it->getY()+offset_y) << ")\n"
				"length = " << it->getL() << "\n"
				"width = " << it->getW() << "\n"
				"\n" <<
				label_lower << " = Microstrip(\n"
				"    pcb=" << it->getSubst() << "_pcb,\n"
				"    position=center,\n"
				"    length=length,\n"
				"    width=width,\n"
				"    propagation_axis=" << rotation_to_axis_pyems_python(it->getR(), false) << ")\n"
				"    \n\n";
	...
	f_out << "# Define port structures \n";
	for(pair<unsigned int, shared_ptr<Element>> it : ports) {
		const string label = it.second->getLabel();
		f_out << "# " << label << " : " << it.second->getType() << "\n" <<
			"port_" << label << "_center = Coordinate2(" << it.second->getX()+offset_x << ", " << -(it.second->getY()+offset_y) << ")\n" <<
			"port_" << label << "_length = " << it.second->getL() << "\n" <<
			"port_" << label << "_width = " << it.second->getW() << "\n" <<
			"port_" << label << "_Z = " << it.second->getZ() << "\n" <<
			"port_" << label << "_P = " << it.second->getDbm() << "\n" <<
			"port_" << label << "_F = " << it.second->getF() << "\n" <<
			"port_" << label << "_excite = args.ports.count(" << it.first
			<< ") == 0\n" << "port_" << label << "_popagation_axis = ("
			<< rotation_to_axis_pyems_python(it.second->getR(), true)
			<< " , " << rotation_to_axis_pyems_python(it.second->getR(), false)
			<< ") [port_" << label << "_excite]\n"
			"\n"
			"port_" << it.second->getN() << " = Microstrip(\n"
			"    pcb=" << it.second->getSubst() << "_pcb,\n"
			"    position=port_" << label << "_center,\n"
			"    length=port_" << label << "_length,\n"
			"    width=port_" << label << "_width,\n"
			"    propagation_axis=port_" << label << "_popagation_axis,\n"
			"    port_number=" << it.second->getN() << ",\n"
			"    excite=port_" << label << "_excite,\n"
			"    feed_shift=0.66,\n"
			"    ref_impedance=port_" << label << "_Z,\n"
			"    measurement_shift=0.7\n"
			")\n\n";

	}

	f_out << "#### Debug structure\n"
		"sim.save_csx()\n"
		"sim.view_csx()\n"
		"\n";
	...
	f_out << "#### Compute mesh\n"
		"mesh = Mesh(\n"
		"    sim=sim,\n"
		"    metal_res=1 / metal_div,\n"
		"    nonmetal_res=1 / substrate_div,\n"
		"    smooth=(1.1, 1.5, 1.5),\n"
		"    min_lines=5,\n"
		"    expand_bounds=((0, 0), (0, 0), (10, 20)),\n"
		"    )\n\n";

	f_out << "#### Set field dumps\n"
		"field_dump = FieldDump(\n"
		"    sim=sim,\n"
		"    box=Box3(\n"
		"        Coordinate3(-pcb_length / 2, -pcb_width / 2, 0),\n"
		"        Coordinate3(pcb_length / 2, pcb_width / 2, 0)),\n"
		"    dump_type=DumpType.current_density_time\n"
		")\n\n";

	f_out << "#### Run simulation\n"
		"\n"
		"sim.run()\n"
		"\n";

	f_out << "####################################################### Post-processing\n"
		"\n";
